		...
		<div class='navbar-inner'>
			<div class='container-fluid'>
				<a class='brand' href='index.html'> <i class='icon-heart-empty'></i>
					<span class='hidden-phone'>中国民用航空飞行学院人事管理系统</span>
		...
		<div class='navigation'>
			...
			<ul class='nav nav-stacked'>
				...
				<li class=''><a class='dropdown-collapse' href='#'> <i
					...
					<ul class='nav nav-stacked'>
						<li class=''><a href='<%=request.getContextPath()%>/BackSite/SignUp.jsp'>
								<!--链接到标签对应的html页面--> <i class='icon-caret-right'></i> <span>报名</span>
						</a></li>
						<li class=''><a href='<%=request.getContextPath()%>/register/listRegister.action'>
								<!--链接到标签对应的html页面--> <i class='icon-caret-right'></i> <span>查看报名表</span>
						</a></li>
						<li class=''><a href='<%=request.getContextPath()%>/BackSite/addGzPlan.jsp'>
								<!--链接到标签对应的html页面--> <i class='icon-caret-right'></i> <span>添加公开招聘计划</span>
						</a></li>
					...
					<div class='row-fluid'>
						<div class='span12'>
							<div class='page-header'>
								<h1 class='pull-left'>
									<i class='icon-edit'></i> <span>培训管理</span>
								</h1>
							</div>
						</div>
					</div>
									...
									<div class="table-responsive">
										<table class="table table-bordered table-hover table-striped">
											...
											<tbody>
												...
												<c:forEach items="${gzPlanList}" var="u">
													<tr>
														<td>${u.title }</td>
														<td>${u.reason }</td>
														<td>${u.academy }</td>
														...
														<td><a
															href="<%=request.getContextPath()%>/gzPlan/showGzPlan.action?gzId=${u.id}">查看</a>
															&nbsp;&nbsp; <a href="javascript:delGzPlan(${u.id })">删除</a>
														</td>
													</tr>
												</c:forEach>
...
<script type="text/javascript" language="javascript">
	function delGzPlan(id) {
		if (confirm("你确定要删除当前选择的信息吗?")) {
           window.location.href='<%=request.getContextPath()%>/gzPlan/delGzPlan.action?gzId='+id;
		}
	}
</script>