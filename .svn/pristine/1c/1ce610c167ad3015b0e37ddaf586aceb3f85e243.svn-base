	...
	<table>
		...
		<c:forEach items="${uList}" var="u">
			<tr>
				...
				<td>${u.password }</td>
				<td><a
					href="<%=request.getContextPath()%>/user/goEditUser.action?userid=${u.userid}">编辑</a>
					&nbsp;&nbsp;<a
					href="<%=request.getContextPath()%>/userServlet?type=view&userid=${u.userid}">查看</a>
...
<script type="text/javascript" language="javascript">
	function deleteUser(id) {
		if (confirm("你确定要删除当前选择的信息吗?")) {
           window.location.href='<%=request.getContextPath()%>/user/delUser.action?userid='+id;
		}
	}
</script>
